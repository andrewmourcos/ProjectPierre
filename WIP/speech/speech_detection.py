...
class Speech():
     def __init__(self):
        self.self.preceed_list = ['and','and(2)','you','or', 'an(2)','ok', 'okay']
        self.self.single_list = ['just','just(2)','normally']
    ...
    def parse(sentence):#,fillers):
        ...
        for word in sentence:
            if word=='like' and (last_word in self.preceed_list):
                iSaid.append(word)
            if word=='basically' and (last_word in self.preceed_list):
                iSaid.append(word)
            if word=='essentially' and (last_word in self.preceed_list):
                iSaid.append(word)
            if (word=='okay' or word=='ok')  and (last_word in self.preceed_list):
                    iSaid.append(word)
            if word=='so' and (last_word in self.preceed_list):
                    iSaid.append(word)
            if (word=='no' or word=='know') and (last_word in self.preceed_list):
                iSaid.append('you know')
            if word=='something' and (last_word in self.preceed_list):
                iSaid.append(word)
            if word in self.single_list:
                    iSaid.append(word)
            if word=='also' and (last_word in self.preceed_list):
                    iSaid.append(word)
            if word=='yeah' and (last_word in self.preceed_list):
                    iSaid.append(word)
    ...
    def run(self)
        ...
        for phrase in self.speech:
            filler, sentence = self.parse(phrase.segments())
